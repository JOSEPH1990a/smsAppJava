...
public abstract class zzbrb<T extends zzbqq> {
    ...
    static <T extends zzbqq> T zza(Class<T> cls) {
        ...
        try {
            try {
                return cls.cast(((zzbrb) Class.forName(str, true, classLoader).getConstructor(new Class[0]).newInstance(new Object[0])).zzamp());
            ...
            while (it.hasNext()) {
                try {
                    arrayList.add(cls.cast(((zzbrb) it.next()).zzamp()));
            ...
            if (arrayList.size() == 1) {
                return (T) ((zzbqq) arrayList.get(0));
            }
            if (arrayList.size() == 0) {
                return null;
            }
            try {
                return (T) ((zzbqq) cls.getMethod("combine", Collection.class).invoke(null, arrayList));

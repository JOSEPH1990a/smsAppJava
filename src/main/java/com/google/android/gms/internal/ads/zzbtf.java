...
public class zzbtf<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public final Map.Entry<K, V> zzfy(int i) {
        return this.zzftk.get(i);
    }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r1v0, resolved type: com.google.android.gms.internal.ads.zzbtf<K extends java.lang.Comparable<K>, V> */
    ...
    public final V put(K k, V v) {
        ...
        int i2 = this.zzftj;
        if (size == i2) {
            zzbtm remove = this.zzftk.remove(i2 - 1);
            zzaot().put((K) ((Comparable) remove.getKey()), (V) remove.getValue());
        }
    ...
    public final V zzfz(int i) {
        zzaos();
        V v = (V) this.zzftk.remove(i).getValue();
        if (!this.zzftl.isEmpty()) {
    ...
    private final int zza(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo((Comparable) this.zzftk.get(size).getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo((Comparable) this.zzftk.get(i2).getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;

...
public final class zzakr {
    ...
    public zzakr(JSONObject jSONObject) throws JSONException {
        if (zzaxz.isLoggable(2)) {
            ...
            zzaxz.v(valueOf.length() != 0 ? "Mediation Response JSON: ".concat(valueOf) : new String("Mediation Response JSON: "));
        }
        ...
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            try {
                ...
                arrayList.add(zzakq);
                if (i < 0) {
                    Iterator<String> it = zzakq.zzdkw.iterator();
                    while (true) {
                        if (it.hasNext()) {
        ...
        if (optJSONObject != null) {
            ...
            this.zzdlx = optLong > 0 ? 1000 * optLong : j;

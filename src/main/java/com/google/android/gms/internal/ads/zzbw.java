...
final class zzbw {
    ...
    static String zza(zzbl zzbl, String str) throws GeneralSecurityException, UnsupportedEncodingException {
        ...
        if (((Boolean) zzwu.zzpz().zzd(zzaan.zzctq)).booleanValue()) {
            ...
            if (zza == null || zza.size() == 0) {
                bArr = zza(zzbuz.zzb(zzc(PlaybackStateCompat.ACTION_SKIP_TO_QUEUE_ITEM)), str, true);
            } else {
                zzbs zzbs = new zzbs();
                ...
                int i = 0;
                while (it.hasNext()) {
                    zzbs.zzhr[i] = zza(it.next(), str, false);
                    i++;
                }
                zzbs.zzhm = zzb(zzb);
                bArr = zzbuz.zzb(zzbs);
    ...
    private static zzbl zzc(long j) {
        zzbl zzbl = new zzbl();
        zzbl.zzeo = Long.valueOf((long) PlaybackStateCompat.ACTION_SKIP_TO_QUEUE_ITEM);
        return zzbl;
    }
    ...
    private static byte[] zza(byte[] bArr, String str, boolean z) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        for (zzcb zzcb : new zzbz().zzqm) {
            zzcb.zza(bArr2, bArr4);
        }

...
public final class zzbff implements zzu<zzbdz> {
    ...
    private static void zza(zzbdk zzbdk, Map<String, String> map) {
        ...
        if (str != null) {
            try {
                ...
                zzaxz.zzeo(String.format("Could not parse buffer parameters in loadControl video GMSG: (%s, %s)", str, str2));
                return;
            }
        }
        if (str2 != null) {
    ...
    public final /* synthetic */ void zza(zzbdz zzbdz, Map map) {
        ...
        if ("background".equals(str)) {
            ...
            try {
                zzbdz2.setBackgroundColor(Color.parseColor(str2));
            ...
            if ("decoderProps".equals(str)) {
                ...
                if (str3 == null) {
                    ...
                    zzbdk.zza(zzbdz2, hashMap);
                }
            } else {
                zzbdq zzabt = zzbdz2.zzabt();
                if (zzabt == null) {
                ...
                if (equals || equals2) {
                    Context context = zzbdz2.getContext();
                    int zza = zza(context, map, "x", 0);
                    int zza2 = zza(context, map, "y", 0);
                    int zza3 = zza(context, map, "w", -1);
                    int zza4 = zza(context, map, "h", -1);
                    int min = Math.min(zza3, zzbdz2.zzacb() - zza);
                    int min2 = Math.min(zza4, zzbdz2.zzaca() - zza2);
                    try {
                ...
                if (zzabu != null) {
                    if ("timeupdate".equals(str)) {
                        ...
                        try {
                            ...
                            zzaxz.zzeo(valueOf.length() != 0 ? "Could not parse currentTime parameter from timeupdate video GMSG: ".concat(valueOf) : new String("Could not parse currentTime parameter from timeupdate video GMSG: "));
                        }
                ...
                if (zzabp2 == null) {
                    ...
                    Context context2 = zzbdz2.getContext();
                    ...
                    try {
                        zzabp2.seekTo((int) (Float.parseFloat(str6) * 1000.0f));
                    } catch (NumberFormatException unused4) {
                        String valueOf2 = String.valueOf(str6);
                        zzaxz.zzeo(valueOf2.length() != 0 ? "Could not parse time parameter from currentTime video GMSG: ".concat(valueOf2) : new String("Could not parse time parameter from currentTime video GMSG: "));
                    }
                    ...
                    Context context3 = zzbdz2.getContext();
                    ...
                    try {
                        zzabp2.setVolume(Float.parseFloat(str7));
                    } catch (NumberFormatException unused5) {
                        String valueOf3 = String.valueOf(str7);
                        zzaxz.zzeo(valueOf3.length() != 0 ? "Could not parse volume parameter from volume video GMSG: ".concat(valueOf3) : new String("Could not parse volume parameter from volume video GMSG: "));
                    }
                } else if ("watermark".equals(str)) {
                    zzabp2.zzabl();
                } else {
                    String valueOf4 = String.valueOf(str);
                    zzaxz.zzeo(valueOf4.length() != 0 ? "Unknown video action: ".concat(valueOf4) : new String("Unknown video action: "));
                }
            }
        }
    }

...
public final class zzabp extends RelativeLayout {
    ...
    public zzabp(Context context, zzabm zzabm, RelativeLayout.LayoutParams layoutParams) {
        super(context);
        ...
        if (zzrp != null && zzrp.size() > 1) {
            ...
            for (zzabr zzabr : zzrp) {
                try {
                    this.zzdap.addFrame((Drawable) ObjectWrapper.unwrap(zzabr.zzsa()), zzabm.zzrq());
                } catch (Exception e) {
            ...
            try {
                imageView.setImageDrawable((Drawable) ObjectWrapper.unwrap(zzrp.get(0).zzsa()));
            } catch (Exception e2) {
